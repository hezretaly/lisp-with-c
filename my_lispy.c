...
lval* builtin_logic(lenv* e, lval* a, char* cond){
    if (strcmp(cond, "not") == 0) {
        LASSERT_NUM(cond, a, 1);
        lval* x = lval_pop(a, 0);
        lval_del(a);
        if(x->num == 0){ x->num=1; return x;}
        x->num = 0;
        return x;
    }
    ...
    while (a->count > 0) {
        lval* x = lval_pop(a, 0);
        if (strcmp(cond, "or") == 0) { if(x->num != 0){
                                        lval_del(a);lval_del(x);
                                        return lval_num(1);} else { continue;}}
        if (strcmp(cond, "and") == 0) {if(x->num == 0){
                                        lval_del(a);lval_del(x);
                                        return lval_num(0);} else { continue;}}
        lval_del(x);
    }
    lval_del(a);
    if (strcmp(cond, "or") == 0) { return lval_num(0);}
    if (strcmp(cond, "and") == 0) { return lval_num(1);}
    return lval_err("builtin_logic function reached the end. Something went wrong!");
}
...
void lenv_add_builtins(lenv* e) {
  ...
  lenv_add_builtin(e, "/", builtin_div);
    ...
    lenv_add_builtin(e, "or", builtin_logic_or);
    lenv_add_builtin(e, "and", builtin_logic_and);
    lenv_add_builtin(e, "not", builtin_logic_not);
